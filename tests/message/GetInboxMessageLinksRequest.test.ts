...
import {GetInboxMessageLinksRequest} from "../../src/message/GetInboxMessageLinksRequest";

describe('GetInboxMessageLinksRequest Tests', function () {
    ...
    )('testGetInboxMessageLinksRequest', async () => {

        const domain = getPrivateDomain();
        const inbox = getInboxTest();
        const message = await postMessage(domain, inbox);

        const request = new GetInboxMessageLinksRequest(domain, inbox, message!.result!.id);
