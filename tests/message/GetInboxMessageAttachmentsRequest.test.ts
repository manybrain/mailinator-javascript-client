...
describe('GetInboxMessageAttachmentsRequest Tests', function () {
    ...
    )('testInboxMessageAttachmentsRequest', async () => {

        const request = new GetInboxMessageAttachmentsRequest(getPrivateDomain(), getInboxTest(), getMessageWithAttachmentId());
    ...
    )('testInboxMessageAttachmentsMissingRequest', async () => {

        const domain = getPrivateDomain();
        const inbox = getInboxTest();
        const message = await postMessage(domain, inbox);

        const request = new GetInboxMessageAttachmentsRequest(domain, inbox, message!.result!.id);
        const response = await request.execute(getApiToken());
