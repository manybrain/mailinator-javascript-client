...
const _resolveTemplateUrl = (domain: string) => {
    return `https://api.mailinator.com/api/v2/domains/${domain}/stream`;
};
...
export class GetStreamDomainMessagesRequest implements Request<Inbox> {
    ...
    execute(apiToken: string): Promise<IRestResponse<Inbox>> {
        ...
        return new Promise((resolve, reject) => {
            httpClient.get(_resolveTemplateUrl(this.domain), _options).then(res => {
